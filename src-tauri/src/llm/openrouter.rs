...
impl LlmProvider for OpenRouterProvider {
    ...
    async fn health_check(&self) -> Result<(), LlmError> {
        ...
        if resp.status().is_success() {
            ...
            Err(LlmError::ProviderUnavailable(
                "OpenRouter not responding".into(),
            ))
        }
    }
    ...
    async fn chat_stream(&self, req: ChatRequest) -> Result<TokenStream, LlmError> {
        ...
        let messages: Vec<OpenRouterMessage> = req
            .messages
            .into_iter()
                                    ...
                                    serde_json::from_str::<OpenRouterStreamChunk>(data)
                                {
                                    if let Some(choice) = chunk.choices.first() {
                                        if let Some(content) = &choice.delta.content {
                                            ...
                                            let _ =
                                                tx.send(TokenEvent::Token(content.clone())).await;
                                        }
                                        if choice.finish_reason.is_some() {
                                            let _ = tx
                                                .send(TokenEvent::Done(ChatResponse {
                                                    ...
                                                    model: chunk.model,
                                                    choices: vec![
                                                        crate::llm::provider::ChatChoice {
                                                            index: 0,
                                                            message:
                                                                crate::llm::provider::ChatMessage {
                                                                    role: "assistant".into(),
